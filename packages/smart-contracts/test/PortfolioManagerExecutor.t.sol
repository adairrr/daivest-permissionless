...
contract PortfolioManagerExecutorTest is Test {
    ...
    function test_Gas_OnInstall() public {
        ...
        console2.log("Gas used for onInstall:", gasUsed);
        assertLt(gasUsed, 200000); // Should use less than 200k gas
    }
    
    function test_Gas_GetAssetPrice() public {
        uint256 gasBefore = gasleft();
        executor.getAssetPrice(address(bnbToken));
        uint256 gasUsed = gasBefore - gasleft();
        
        console2.log("Gas used for getAssetPrice:", gasUsed);
        assertLt(gasUsed, 50000); // Should use less than 50k gas
    }
    
    function test_Gas_CalculatePortfolioValue() public {
        bytes memory installData = abi.encode(assets, allocations, rebalancingThreshold);
        executor.onInstall(installData);
        
        uint256 gasBefore = gasleft();
        executor.calculatePortfolioValue(address(mockAccount));
        uint256 gasUsed = gasBefore - gasleft();
        
        console2.log("Gas used for calculatePortfolioValue:", gasUsed);
